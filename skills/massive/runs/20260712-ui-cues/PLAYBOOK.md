...
## 6. Harvest (preferred path)

1. Confirm finished (no Stop; action bar visible).  
2. **Scroll through the entire response** (multiple screenshots if needed) until the **bottom action bar** is on screen and you’ve seen the full answer.  
3. Click **assistant `Copy`** (real cursor or verified element index after fresh snapshot).  
4. `pbpaste` → write `*-response.txt`.  
5. Verify clipboard content is the reply (not the user prompt, not empty).

**Fallback only:** `document.body.innerText` via Safari AppleScript — noisier (chrome, chips, sidebar). Prefer Copy.
