...
function reexecWithLocalGranolaIfNeeded() {
  if (process.env.PRESENTATIONS_RUNTIME !== "local-granola") return;
  if (process.env.PRESENTATIONS_LOCAL_GRANOLA_TSX === "1") return;

  const scriptsDir = path.dirname(fileURLToPath(import.meta.url));
  const wrapper = path.join(scriptsDir, "run_with_local_granola.mjs");
  const targetScript = process.argv[1] ? path.resolve(process.argv[1]) : undefined;
  if (!targetScript || targetScript === wrapper) return;
  if (process.env.PRESENTATIONS_LOCAL_GRANOLA_REEXEC === "1") {
    throw new Error("PRESENTATIONS_RUNTIME=local-granola requires run_with_local_granola.mjs or tsx.");
  }

  const result = spawnSync(process.execPath, [wrapper, targetScript, ...process.argv.slice(2)], {
    cwd: process.cwd(),
    env: {
      ...process.env,
      PRESENTATIONS_LOCAL_GRANOLA_REEXEC: "1",
    },
    stdio: "inherit",
  });
  if (result.error) {
...
export async function ensureArtifactToolWorkspace(workspaceDir) {
  if (process.env.PRESENTATIONS_RUNTIME === "local-granola") {
    reexecWithLocalGranolaIfNeeded();
    const { ensureGranolaWorkspace } = await import("./granola_artifact_tool_compat.mjs");
    return ensureGranolaWorkspace(workspaceDir);
  }
...
export async function importArtifactTool(workspaceDir) {
  if (process.env.PRESENTATIONS_RUNTIME === "local-granola") {
    reexecWithLocalGranolaIfNeeded();
    const { importGranolaCompat } = await import("./granola_artifact_tool_compat.mjs");
    return importGranolaCompat(workspaceDir);
  }
