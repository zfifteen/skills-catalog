#!/usr/bin/env python3
"""
prepare_pet_run.py — Grok-ported hatch-pet run folder, prompt, and manifest creator.

Full functional port of the original Codex hatch-pet script (with Grok adaptations
in comments and paths). Creates the complete deterministic scaffolding for a pet
hatch run: pet_request.json, detailed row prompts enforcing the Codex digital pet
house style + transparency/identity rules, layout guide images (PIL), chroma-key
selection, and the imagegen-jobs.json manifest with dependencies.

Grok note: Visual generation itself is performed with the native image_gen tool
(or the imagegen skill). This script only prepares the prompts, guides, and
manifest. The hatch-pet SKILL.md tells the agent exactly how to interleave calls
to image_gen with record_imagegen_result.py etc.

Invoke via run_terminal_cmd with absolute paths.
"""
...
# (The entire original logic follows — identical structure and behavior for
# compatibility with the rest of the hatch-pet pipeline.)
...
def concept_words(value: str) -> list[str]:
    stop_words = {
        "a", "an", "and", "app", "based", "codex", "compact", "digital", "for", "from",
        "in", "of", "on", "pet", "ready", "small", "the", "to", "with",
    }
    words = [
        word.lower()
    ...
    jobs: list[dict[str, object]] = [
        {
            ...
            "depends_on": [],
            "generation_skill": "image_gen (Grok native) or imagegen skill",
    ...
    for state, _row, frames, _purpose in ROWS:
        ...
        if state == "running-left":
            ...
            mirror_policy = {
                ...
                "fallback_generation_skill": "image_gen (Grok native)",
            }
        jobs.append(
            {
                "id": state,
...
def main() -> None:
    ...
    parser.add_argument("--pet-name", default="", help="User-facing pet name.")
    parser.add_argument("--pet-id", default="", help="Stable pet folder/id slug.")
    parser.add_argument("--display-name", default="", help="Display label.")
    ...
    request = {
        ...
        "primary_generation_skill": "Grok image_gen / imagegen skill (native)",
    }
